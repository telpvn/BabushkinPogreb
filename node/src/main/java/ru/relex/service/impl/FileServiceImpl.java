...
public class FileServiceImpl implements FileService {
    ...
    public AppDocument processDoc(Message telegramMessage) {
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            BinaryContent persistentBinaryContent = getPersistenBinaryContent(response);
            AppDocument transientAppDoc = buildTransientAppDoc(telegramDoc, persistentBinaryContent);
            return appDocumentDAO.save(transientAppDoc);
        } else {
            throw new UploadFileException("Bad response telegram service: " + response);
        }
    }

    @Override
    public AppPhoto processPhoto(Message telegramMessage) {
        //TODO пока, что обрабатываем одно фото в сообщении
    ...
    private BinaryContent getPersistenBinaryContent(ResponseEntity<String> response) {
